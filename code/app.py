...
mydb = pymysql.connect(
    host='mubn000.mysql.pythonanywhere-services.com',
    user='mubn000',
    password='syedmubn',
    port= 3306,
    database= 'mubn000$voting_system',
    connect_timeout=10,  # Connection timeout in seconds
    read_timeout= 30,     # Read timeout for executing queries
)

## Ensure the upload directory exists
upload_folder = '/home/mubn000/voting-app/e-votings-app/code/static/student_images'
...
def add_candidate():
    if request.method == 'POST':
        try:
            ...
            if not all([position, member_name, party_name, symbol_name, cnic]):
                flash("All fields are required.", 'danger')
                return redirect(url_for('add_candidate'))

            if not re.match(CNIC_PATTERN, cnic):
                flash("Invalid CNIC format. Use 00000-0000000-0 format.", 'danger')
                return redirect(url_for('add_candidate'))
...
def delete_candidate():
    # Log the request method to confirm it matches the intended HTTP method
    ...
    if request.method == 'POST':
        try:
            ...
            if not re.match(CNIC_PATTERN, cnic):
                flash("Invalid CNIC format. Use 00000-0000000-0 format.", 'danger')
                return redirect(url_for('delete_candidate'))

            # Database deletion code (assuming mydb and cursor are set up)
            cur = mydb.cursor()
            cur.execute("DELETE FROM candidates WHERE cnic = %s", (cnic,))
            mydb.commit()
    ...
    # Render the deletion form for GET requests
    return render_template('delete_candidate.html')
# @app.route('/delete_candidate_page')
...
predictor = dlib.shape_predictor('/home/mubn000/voting-app/e-votings-app/code/shape_predictor_68_face_landmarks.dat')  # Download from dlib's website
